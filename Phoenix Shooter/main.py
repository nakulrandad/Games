...
LIFE = pygame.transform.scale(pygame.image.load(os.path.join("assets", "life_trans.png")), (45, 45))
...
def main():
    ...
    def redraw_window():
        ...
        for player in (players + dead_players): 
            ...
            WIN.blit(score_label, ( WIDTH/2 - score_label.get_width()/2 , 10 + (player.player_id-1)*28))
            lives_label = main_font.render(f"Player {player.player_id} - Lives: {player.lives}", 1, (255,255,255))
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, WIDTH-100), random.randrange(-HEIGHT*2, -100), random.choice(["red", "blue", "green"]))
        ...
        if keys[pygame.K_d] and player1.x + player1.vel + player1.get_width() < WIDTH: # right
            player1.x += player1.vel
        if keys[pygame.K_w] and player1.y - player1.vel > 0: # up
            player1.y -= player1.vel
        if keys[pygame.K_s] and player1.y + player1.vel + player1.get_height() + 15< HEIGHT: # down
            player1.y += player1.vel
        ...
        if num_of_players == 2:
            ...
            if keys[pygame.K_RIGHT] and player2.x + player2.vel + player2.get_width() < WIDTH: # right
                player2.x += player2.vel
            if keys[pygame.K_UP] and player2.y - player2.vel > 0: # up
                player2.y -= player2.vel
            if keys[pygame.K_DOWN] and player2.y + player2.vel + player2.get_height() + 15< HEIGHT: # down
                player2.y += player2.vel
        ...
        if random.randrange(0, 80*FPS) == 5:
            powerup1 = Powerup(random.randrange(30, WIDTH-60), random.randrange(-HEIGHT, -50), "life")
            powerups.append(powerup1)

        if random.randrange(0, 60*FPS) == 3:
            powerup1 = Powerup(random.randrange(30, WIDTH-60), random.randrange(-HEIGHT, -50), "heart")
            powerups.append(powerup1)

        if random.randrange(0, 60*FPS) == 7:
            powerup1 = Powerup(random.randrange(30, WIDTH-60), random.randrange(-HEIGHT, -50), "electric")
            powerups.append(powerup1)
...
def main_menu():
    ...
    while run:
        ...
        title_label = title_font.render("Press 1 for one player and 2 for two...", 1, (255,255,255))
        WIN.blit(title_label, (int((WIDTH - title_label.get_width())/2), int(350/750 * HEIGHT)))
        pygame.display.update()
        keys = pygame.key.get_pressed()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
